 ...
 * @Last Modified time: 2021-03-09 20:27:18
 */
...
class Auth {
	...
	public function confirm($db, $user_id, $token){
		...
		if($user && $user->confirmation_token == $token ){
			$db->query('UPDATE users SET confirmation_token=?, confirmation_at = NOW() WHERE id = ?', [null, $user_id]);
	...
	public function addToNewsletter($db, $email){
		return $db->query("INSERT INTO newsletter SET email=?, date=Now()", [$email]);
	}
